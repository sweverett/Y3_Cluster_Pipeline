...
void* parse_config(void *config) {
    ...
    return new artisanal_pdf_config {
...
void delete_config(void *config) {
    delete reinterpret_cast<const artisanal_pdf_config *>(config);
}

// original pdf DO NOT STEAL!
double artisanal_pdf(double x, double y, void *config) {
    const auto conf = *reinterpret_cast<const artisanal_pdf_config *>(config);
...
// function which loads config from some file
...
int main() {
    ...
    auto result = do_integral(artisanal_pdf, config);
